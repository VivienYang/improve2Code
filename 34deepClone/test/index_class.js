...
describe("DeepCloner",()=>{
  it('是一个函数',()=>{
    assert.isFunction(new DeepCloner().deepClone)
  })
  it("可以复制基本类型",()=>{
    let num = 1
    assert(num === new DeepCloner().deepClone(num))
    let str = 'ahhk'
    assert(str === new DeepCloner().deepClone(str))
    let bool = false
    assert(bool === new DeepCloner().deepClone(bool))
    let u = undefined
    assert(u === new DeepCloner().deepClone(u))
    let empty = null
    assert(empty === new DeepCloner().deepClone(empty))
    let syb = Symbol()
    assert(syb === new DeepCloner().deepClone(syb))
  })
  describe("可以复制对象",()=>{
    it("能够复制普通对象",()=>{
    ...
    it("能够复制数组",()=>{
      ...
      const arr2 = new DeepCloner().deepClone(arr)
      assert(arr !== arr2)
      assert(arr[0] !== arr2[0])
      assert(arr[1] !== arr2[1])
      assert(arr[2] !== arr2[2])
    ...
    it("能够复制函数",()=>{
      ...
      f1.xxx ={yyy:{zzz:1}}
      const f2=new DeepCloner().deepClone(f1)
      assert(f1 !== f2)
    ...
    it("能够复制环",()=>{
      const a = {name:'sshh'}
      a.self = a
      const a2 = new DeepCloner().deepClone(a)
      assert(a !== a2)
